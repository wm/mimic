...
import (
	...
	"github.com/wm/mimic/icis"
	"github.com/wm/mimic/utils"
)

type Config struct {
	Applications []icis.App   `json:"apps"`
	Tunnel       utils.Tunnel `json:"tunnel"`
}

func (cfg *Config) Apps() []application.App {
	var apps []application.App
...
func getConfigFile() ([]byte, error) {
	cfgFile, _ := syscall.Getenv("STAGING_DUMP_CONFIG_FILE")
	home, ok := syscall.Getenv("HOME")
	if ok && (len(cfgFile) < 1) {
		cfgFile = home + "/src/toolchest/go/staging_dump/config.json"
	}
